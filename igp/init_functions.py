...
def update_tz_data():
    ...
    df_region.to_csv(f"{app.config['TMP_FOLDER']}\\tz.csv", sep=';', index=False)

    return os.path.basename(f"{app.config['TMP_FOLDER']}\\tz.csv")
...
data_tz = pd.read_csv(f"{app.config['TMP_FOLDER']}\\tz.csv", delimiter=';')
